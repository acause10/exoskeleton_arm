...
namespace ExoControllers{
    ...
    double PosControl::YrTheta(double q1, double qd1, double qd1r, double qdd1r)
    {
        ...
        Yr(0,0) = qdd1r;
        Yr(0,1) = qd1r;
        Yr(0,2) = -m_gx*sin(q1)+m_gy*cos(q1);
        Yr(0,3) =  q1;
        Yr(0,4) = 1;

        Theta(0,0) = m_I233 + (pow(m_L2,2)*m_m2)/4;
        Theta(1,0) = m_b1;
        Theta(2,0) = (m_L2*m_m2)/2; 
        Theta(3,0) = m_k1;
        Theta(4,0) = -m_k1*m_theta1;
    ...
    double PosControl::update(double delta_t, double q1, double qd1, double qdd1)
    {
        ...
        double deltaQd = qd1 - m_qd_des; // to Do! //CL Form just in case .. previous: qd1 - m_qd_des
        ...
        //ROS_WARN_STREAM("Sq: " << Sq);
        //ROS_WARN_STREAM("qd1: " << qd1);
        //ROS_WARN_STREAM("m_tao: " << m_tao);
        //ROS_WARN_STREAM("Regressor: " << YrTheta(q1, qd1, qd1r, qdd1r));

        return m_tao;
    }

    Vector3d PosControl::getter(){

        Vector3d qEnd;
        qEnd << m_q_des, m_qd_des, m_qdd_des;
